...
/* base instruction that will calculate user pending rewards and distribute SOL's 
according to it, on the action of Stake, Unstake or Claim.  */

#[derive(Accounts)]
pub struct Base<'info> {
    // User that is performing action
    #[account(mut)]
...
impl<'info> Base<'info> {
    ...
    // Check will fund controller left with enough SOL after transfering the reward amount.
    ...
    // Calculate total pending reward of user, that is accumulate on his staked $MESH & $indexMESH.
    ...
    // Calulate pending reward, distribute that reward and update the states.
